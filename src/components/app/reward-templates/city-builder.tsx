  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex h-72 items-end gap-1.5 overflow-x-auto rounded-card border border-subtle bg-bg-base p-4">
        {buildings.map((_, i) => {
          const built = i < progress.masteredLessons;
          ...
          return (
            <div
              ...
              className={cn(
                ...
                built ? "bg-brand-500 shadow-glow-brand" : "bg-bg-surface border border-strong"
              )}
              style={{ height: h }}
            >
              {/* نوافذ */}
      ...
      <p className="text-center text-secondary text-text-secondary">
        بنيت {progress.masteredLessons} من {progress.totalLessons} مبنى
      </p>
    </div>
  );
}
