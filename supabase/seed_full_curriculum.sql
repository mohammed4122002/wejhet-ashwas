...
insert into subjects (track, name_ar, slug, order_index) values
  ...
  ('literary',   'اللغة الإنجليزية (أدبي 2)',  'english-literary-2', 26);
...
select s.id, v.name_ar, v.idx from subjects s
cross join (values
  ('التركيب الذري والجدول الدوري', 1),
  ('الأحماض والقواعد', 2),
  ('الديناميكا الحرارية', 3),
  ('سرعة التفاعل الكيميائي', 4),
  ('الكيمياء العضوية', 5),
  ('الكيمياء الكهربائية', 6)
) as v(name_ar, idx)
...
cross join lateral (values
  ('التركيب الذري والجدول الدوري', 'الضوء مفتاح البناء الإلكتروني', 1),
  ('التركيب الذري والجدول الدوري', 'الطيف الذري', 2),
  ('التركيب الذري والجدول الدوري', 'نظرية بور لذرة الهيدروجين', 3),
  ('التركيب الذري والجدول الدوري', 'نظرية الميكانيك الكمي (الموجي)', 4),
  ('التركيب الذري والجدول الدوري', 'قواعد التركيب الإلكتروني', 5),
  ('التركيب الذري والجدول الدوري', 'العدد الذري وإلكترونات التكافؤ', 6),
  ('التركيب الذري والجدول الدوري', 'الجدول الدوري الحديث', 7),
  ('التركيب الذري والجدول الدوري', 'الخصائص الدورية للعناصر', 8),
  ('التركيب الذري والجدول الدوري', 'العناصر الانتقالية في الدورة الرابعة من الجدول الدوري', 9),
  ('التركيب الذري والجدول الدوري', 'نظرية رابطة التكافؤ', 10),
  ('الأحماض والقواعد', 'الخواص العامة للحموض والقواعد', 1),
  ('الأحماض والقواعد', 'تطوير مفهومي الحمض والقاعدة', 2),
  ('الأحماض والقواعد', 'التأين الذاتي للماء والرقم الهيدروجيني', 3),
  ('الأحماض والقواعد', 'الاتزان في محاليل الحموض والقواعد الضعيفة', 4),
  ('الأحماض والقواعد', 'الخواص الحمضية والقاعدية لمحاليل الأملاح', 5),
  ('الأحماض والقواعد', 'الأيون المشترك والمحلول المنظم', 6),
  ('الأحماض والقواعد', 'المعايرة بين الحموض والقواعد', 7),
  ('الديناميكا الحرارية', 'الديناميكا الحرارية', 1),
  ('الديناميكا الحرارية', 'العمليات التلقائية والعمليات غير التلقائية', 2),
  ('الديناميكا الحرارية', 'العشوائية', 3),
  ('الديناميكا الحرارية', 'القانون الثاني للديناميكا الحرارية', 4),
  ('الديناميكا الحرارية', 'طاقة جبس الحرة', 5),
  ('سرعة التفاعل الكيميائي', 'سرعة التفاعل الكيميائي وميكانيكيته', 1),
  ('سرعة التفاعل الكيميائي', 'قانون سرعة التفاعل الكيميائي', 2),
  ('سرعة التفاعل الكيميائي', 'نظرية الحالة الانتقالية', 3),
  ('سرعة التفاعل الكيميائي', 'آلية التفاعل', 4),
  ('الكيمياء العضوية', 'المركبات العضوية', 1),
  ('الكيمياء العضوية', 'هاليدات الألكيل', 2),
  ('الكيمياء العضوية', 'الكحولات', 3),
  ('الكيمياء العضوية', 'الألدهيدات والكيتونات', 4),
  ('الكيمياء العضوية', 'الحموض الكربوكسيلية', 5),
  ('الكيمياء الكهربائية', 'الخلايا الجلفانية', 1),
  ('الكيمياء الكهربائية', 'تركيب الخلايا الجلفانية وعملها', 2),
  ('الكيمياء الكهربائية', 'جهد القطب القياسي', 3),
  ('الكيمياء الكهربائية', 'حساب جهد الخلايا الجلفانية', 4)
) as v(unit_name, name_ar, idx)
where u.name_ar = v.unit_name;
...
cross join lateral (values
  ('عمليات حيوية في الخلية', 'أهمية الطاقة للخلية الحية', 1),
  ('عمليات حيوية في الخلية', 'البناء الضوئي', 2),
  ('عمليات حيوية في الخلية', 'التنفس الخلوي', 3),
  ('عمليات حيوية في الخلية', 'التكامل بين عمليتي البناء الضوئي والتنفس الخلوي', 4),
  ('الوراثة', 'الشيفرة الوراثية', 1),
  ('الوراثة', 'بناء البروتين', 2),
  ('الوراثة', 'الوراثة المندلية', 3),
  ('الوراثة', 'أنماط التوارث غير المندلية', 4),
  ('الوراثة', 'السيادة غير التامة', 5),
  ('الوراثة', 'السيادة المشتركة', 6),
  ('الوراثة', 'الأليلات المتعددة', 7),
  ('الوراثة', 'الجينات القاتلة', 8),
  ('الوراثة', 'الصفات الوراثية', 9),
  ('الوراثة', 'أنظمة تحديد الجنس في الكائنات الحية', 10),
  ('الوراثة', 'الصفات المرتبطة بالجنس', 11),
  ('الوراثة', 'الصفات المتأثرة بالجنس', 12),
  ('الوراثة', 'الارتباط والعبور', 13),
  ('الوراثة', 'الخرائط الجينية', 14),
  ('الوراثة', 'الاختلالات الوراثية', 15),
  ('أجهزة جسم الإنسان', 'وظائف الجهاز الهيكلي', 1),
  ('أجهزة جسم الإنسان', 'أقسام الجهاز الهيكلي', 2),
  ('أجهزة جسم الإنسان', 'أشكال العظام', 3),
  ('أجهزة جسم الإنسان', 'تركيب نسيج العظم', 4),
  ('أجهزة جسم الإنسان', 'تركيب جهاز الدوران', 5),
  ('أجهزة جسم الإنسان', 'تخثر الدم والتئام الجروح', 6),
  ('أجهزة جسم الإنسان', 'أمراض تصيب جهاز الدوران', 7),
  ('أجهزة جسم الإنسان', 'الأنظمة المناعية في الجسم', 8),
  ('أجهزة جسم الإنسان', 'المناعة الإيجابية والمناعة السلبية', 9),
  ('أجهزة جسم الإنسان', 'تركيب الأجسام المضادة ووظائفها', 10),
  ('أجهزة جسم الإنسان', 'الاختلالات المناعية', 11),
  ('الكائنات الدقيقة', 'أماكن تواجد البكتيريا', 1),
  ('الكائنات الدقيقة', 'تصنيف بدائية النوى', 2),
  ('الكائنات الدقيقة', 'أشكال البكتيريا', 3),
  ('الكائنات الدقيقة', 'تركيب الخلية البكتيرية', 4),
  ('الكائنات الدقيقة', 'تركيب الفيروسات', 5),
  ('الكائنات الدقيقة', 'أشكال الفيروسات', 6),
  ('الكائنات الدقيقة', 'تصنيف الفيروسات', 7)
) as v(unit_name, name_ar, idx)
where u.name_ar = v.unit_name;
...
-- ╔══════════════════════════════════════════════════════════════════╗
-- ║              اللغة الإنجليزية (أدبي 2) — track أدبي فقط          ║
-- ╚══════════════════════════════════════════════════════════════════╝

insert into units (subject_id, name_ar, order_index)
select s.id, v.name_ar, v.idx from subjects s
cross join (values
  ('Unit 1: Making Changes', 1),
  ('Unit 2: Money Talks', 2),
  ('Unit 3: Past History?', 3)
) as v(name_ar, idx)
where s.slug = 'english-literary-2';

insert into lessons (unit_id, name_ar, order_index)
select u.id, v.name_ar, v.idx from units u
join subjects s on s.id = u.subject_id and s.slug = 'english-literary-2'
cross join lateral (values
  ('Unit 1: Making Changes', 'New Year customs in different countries', 1),
  ('Unit 1: Making Changes', 'Making and keeping resolutions', 2),
  ('Unit 1: Making Changes', 'Dealing with unfamiliar vocabulary', 3),
  ('Unit 1: Making Changes', 'Question words with the suffix -ever', 4),
  ('Unit 1: Making Changes', 'Compound adjectives and verbs', 5),
  ('Unit 2: Money Talks', 'The positives and negatives of money', 1),
  ('Unit 2: Money Talks', 'Headlines', 2),
  ('Unit 2: Money Talks', 'Synonyms', 3),
  ('Unit 2: Money Talks', 'Rich/poor divide', 4),
  ('Unit 2: Money Talks', 'A history of money', 5),
  ('Unit 2: Money Talks', 'Noun phrases with the + adjective', 6),
  ('Unit 3: Past History?', 'Different views of history', 1),
  ('Unit 3: Past History?', 'The Palestinian Nakba and Diaspora', 2),
  ('Unit 3: Past History?', 'The work of President Yasser Arafat and Palestinian intellectuals', 3),
  ('Unit 3: Past History?', 'Non-literal or figurative language', 4)
) as v(unit_name, name_ar, idx)
where u.name_ar = v.unit_name;
